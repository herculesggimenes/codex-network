...
function integerEnv(name, fallback, { min = 0 } = {}) {
  ...
  if (!Number.isInteger(value) || value < min) {
    throw new Error(`${name} must be an integer greater than or equal to ${min}: ${raw}`);
  }
  return value;
}
...
function messageFromError(error) {
  if (error instanceof Error) {
    return process.env.CODEX_NETWORK_DEBUG && error.stack ? error.stack : error.message;
  }
  return String(error);
}
...
function itemText(item) {
  ...
  if (typeof item.text === "string" && (item.type || "").toLowerCase().includes("agent"))
    return item.text;
  return null;
}
...
class CodexRpcClient {
  ...
  async connect() {
    if (this.timeoutMs > 0) {
      this.timeout = setTimeout(() => {
        const error = new Error(`Codex RPC timed out after ${this.timeoutMs}ms`);
    ...
    await new Promise((resolve, reject) => {
      ...
      const onError = (event) => {
        cleanup();
        reject(this.eventError(event, "websocket connection failed"));
      };
    ...
    this.ws.addEventListener("error", (event) => {
      if (!this.closing) this.rejectPending(this.eventError(event, "websocket error"));
    });
    this.ws.addEventListener("close", () => {
      if (!this.closing) this.rejectPending(new Error("websocket closed"));
    });
  ...
  eventError(event, fallback) {
    return event?.error instanceof Error ? event.error : new Error(fallback);
  }
...
async function listThreads() {
  ...
  const limit = integerEnv("CODEX_NETWORK_LIMIT", 20, { min: 1 });
...
function printMatch(nodeName, thread) {
  console.log(
    [nodeName, thread.id, isoTime(thread.updatedAt), thread.cwd || "", titleOf(thread)]
      .map(field)
      .join("\t"),
  );
}
...
async function resolveThread() {
  ...
  const maxScan = integerEnv("CODEX_NETWORK_MAX_SCAN", 2000, { min: 1 });
...
async function sendMessage() {
  ...
  const client = new CodexRpcClient(url, {
    timeoutMs: integerEnv("CODEX_NETWORK_SEND_TIMEOUT_MS", 0),
    capabilities: {
      optOutNotificationMethods: ["item/reasoning_text/delta", "item/reasoning_summary_text/delta"],
    },
  });
  await client.connect();
  ...
  const activeTurn = Array.isArray(resume?.thread?.turns)
    ? resume.thread.turns.findLast((turn) => turn?.status === "inProgress")
    : null;
